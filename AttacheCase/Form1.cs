...
namespace AttacheCase
{
  public partial class Form1 : Form
  {
    ...
    // ライトテーマ配色（半透明対応: 純白を避け青みのある乳白色を使用）
    // 調整用: #EEF3F8 + Opacity 0.93 をベースに検証する
    private bool _isDarkTheme = false; // 現在のテーマ状態（BackColor 判定の代替）
    private static readonly Color LightBaseColor = Color.FromArgb(0xEE, 0xF3, 0xF8);       // #EEF3F8 フォーム・パネル背景
    private static readonly Color LightSidebarColor = Color.FromArgb(0xD8, 0xE1, 0xED);     // #D8E1ED サイドバー
    private static readonly Color LightHeaderColor = Color.FromArgb(0xE2, 0xE8, 0xF0);      // #E2E8F0 ヘッダーパネル
    private static readonly Color LightContentColor = Color.FromArgb(0xF5, 0xF8, 0xFC);     // #F5F8FC コンテンツ領域（可読性優先）
    private static readonly Color LightHoverColor = Color.FromArgb(0xE8, 0xEF, 0xF5);       // #E8EFF5 マウスホバー
    private static readonly Color LightDragColor = Color.FromArgb(0xE0, 0xF0, 0xE4);        // #E0F0E4 ドラッグ時
    private const double LightOpacity = 0.93;

    // ダークテーマ配色
    private static readonly Color DarkSidebarColor = Color.FromArgb(255, 30, 30, 30);       // #1E1E1E サイドバー
    private static readonly Color DarkHeaderColor = Color.FromArgb(255, 35, 35, 35);        // #232323 ヘッダーパネル
    private static readonly Color DarkHoverColor = Color.FromArgb(255, 40, 40, 40);         // #282828 マウスホバー
    private static readonly Color DarkDragColor = Color.FromArgb(255, 50, 50, 50);          // #323232 ドラッグ時

    // ─── アイコンベース画像（白シルエットPNG） ───
    private Bitmap _iconEncrypt;    // menu_encrypt.png - 暗号化(ATC/EXE共用)
    private Bitmap _iconDecrypt;    // menu_decrypt.png - 復号
    private Bitmap _iconPublicKey;  // menu_public_key.png - RSA/公開鍵
    private Bitmap _iconBack;       // menu_back.png - 戻るボタン
    private Bitmap _iconSettings;   // menu_settings.png - 設定
    private Bitmap _iconPostPpap;   // menu_post_ppap.png - Post-PPAP
    private Bitmap _iconMailAddress;  // resx 埋め込み - メールアドレスアイコン
    private Bitmap _iconCheckIcon;    // resx 埋め込み - チェックマーク
    private Bitmap _iconLockIcon;     // resx 埋め込み - ロック
    private Bitmap _iconInvalidIcon;  // resx 埋め込み - 無効（警告）
    private Bitmap _iconKeyIcon;      // resx 埋め込み - 鍵

    // ─── サイドバーアイコン選択状態 ───
    private bool _atcChecked = false;
    private bool _exeChecked = false;
    private bool _rsaChecked = false;
    private bool _decChecked = false;
    ...
    private FileEncrypt5 encryption5;
    private FileDecrypt4 decryption4;     // ヘッダ読み込み・v4復号用に残す
    private FileDecrypt5 decryption5;   // v5復号用
    private Wipe wipe;
    ...
    public Form1()
    {
      ...
      panelPostPpapRegister.Parent = panelOuter;
      panelPostPpapMain.Parent = panelOuter;
      ...
      {
        this.BackColor = LightBaseColor;
        ChangeTheme(this.Controls, false);
      }
      
      // 商用ライセンスのチェック
      var lr = new LicenseRegister();
      if (lr.Decrypt(false))
      {
        // 商用ライセンスは非表示
        toolStripStatusLabelLicense.Text = string.Empty;
      }
      else if (lr.GetAcademicLicense())
      {
        // 学生・教職員向けライセンス
        // ---
        // Academic License
        toolStripStatusLabelLicense.Text = Resources.DialogMessageAcademicLicense;
      }
      else
      {
        // 個人利用
        // --
        // Personal Use
        toolStripStatusLabelLicense.Text = Resources.DialogMessagePersonalUse;
      }
    ...
    private void Form1_Load(object sender, EventArgs e)
    {
      ...
      panelPostPpapRegister.Visible = false;
      panelPostPpapMain.Visible = false;
    ...
    private void Form1_Shown(object sender, EventArgs e)
    {
      ...
      // 左サイドバーアイコンメニューの位置決め
      pictureBoxAtc.Location = new Point(10,10);
      labelAtc.Location = new Point(10, pictureBoxAtc.Bottom + 2);
      labelAtc.Text = Resources.LabelAtc;
      
      pictureBoxDec.Location = new Point(10, labelAtc.Bottom + 8);
      labelDec.Location = new Point(10, pictureBoxDec.Bottom + 2);
      labelDec.Text = Resources.LabelDec;

      pictureBoxRsa.Location = new Point(10, labelDec.Bottom + 8);
      labelRsa.Location = new Point(10, pictureBoxRsa.Bottom + 2);
      labelRsa.Text = Resources.LabelRsa;

      pictureBoxPostPpap.Location = new Point(10, labelRsa.Bottom + 8);
      labelPostPpap.Location = new Point(10, pictureBoxPostPpap.Bottom + 2);
      labelPostPpap.Text = Resources.LabelPostPpap;

      // Post-PPAPのドラッグ＆ドロップエリアのトップ位置を固定する
      panelPublicDragAndDrop.Top = labelValidDetails.Top + labelValidDetails.Height + 16;
      // Post-PPAPのドラッグ＆ドロップエリアの枠線描画イベントを追加
      panelPublicDragAndDrop.Paint += panelPublicDragAndDrop_Paint;
      // サイズが変わったときに再描画するように設定
      panelPublicDragAndDrop.Resize += (s, e) => panelPublicDragAndDrop.Invalidate();
      // ドラッグ＆ドロップエリアのマウスイベント（カラーチェンジ）
      panelPublicDragAndDrop.MouseEnter += panelPublicDragAndDrop_MouseEnter;
      panelPublicDragAndDrop.MouseLeave += panelPublicDragAndDrop_MouseLeave;
      // 初期状態では非表示（検索前）
      panelPublicDragAndDrop.Visible = false;
      // 検索結果ラベル・アイコンも非表示
      labelValidMessage.Visible = false;
      labelValidDetails.Visible = false;
      pictureBoxValid.Visible = false;
      pictureBoxValidDetails.Visible = false;

      // ウィンドウ表示後にバックドロップを適用（ダーク・ライト両対応）
      ApplyBackdrop(AppSettings.Instance.CurrentThemeColorName == "dark");
    ...
    /// <summary>
    /// 現在表示中のパネルのレイアウトを再計算する。
    /// VisibleChanged / Resize の両方から呼び出される一元化メソッド。
    /// </summary>
    private void LayoutActivePanel()
    {
      if (panelStartPage.Visible) LayoutPanelStartPage();
      else if (panelEncrypt.Visible) LayoutPanelEncrypt();
      else if (panelEncryptConfirm.Visible) LayoutPanelEncryptConfirm();
      else if (panelDecrypt.Visible) LayoutPanelDecrypt();
      else if (panelRsa.Visible) LayoutPanelRsa();
      else if (panelRsaKey.Visible) LayoutPanelRsaKey();
      else if (panelPostPpapRegister.Visible) LayoutPanelPostPpapRegister();
      else if (panelPostPpapMain.Visible) LayoutPanelPostPpapMain();
      else if (panelProgressState.Visible) LayoutPanelProgressState();
    }

    /// <summary>
    /// メインフォーム内のスタートページパネルのレイアウトを設定および配置します。
    /// </summary>
    /// <remarks>
    /// 初期化、およびフォームのリサイズイベントで呼び出され、
    /// スタートページのコントロールを適切に配置します。
    /// 
    /// </remarks>
    private void LayoutPanelStartPage()
    {
      labelDragAndDrop.Top = panelStartPage.Height / 2 - labelDragAndDrop.Height / 2;
      int contentWidth = panelStartPage.Width - panelSidebarStart.Width;
      labelDragAndDrop.Left = panelSidebarStart.Width + contentWidth / 2 - labelDragAndDrop.Width / 2;
      // 設定アイコン
      pictureBoxOption.Location = new Point(panelSidebarStart.Width + 8, panelStartPage.Height - 38);
    }

    /// <summary>
    /// 暗号化パネルのレイアウトを設定します。
    /// テキストボックス、ラベル、ボタンなどのコントロールの位置やサイズを調整します。
    /// </summary>
    private void LayoutPanelEncrypt()
    {
      // 左サイドバー
      pictureBoxEncryption.Left = 0;
      pictureBoxEncryption.Top = panelSidebarEncrypt.Height / 2 - pictureBoxEncryption.Height / 2;
      labelEncryption.Left = 0;
      labelEncryption.Top = pictureBoxEncryption.Bottom + 2;

      // レイアウトの基準（パスワード入力テキストボックス）
      textBoxPassword.Left = panelSidebarEncrypt.Width + 8;
      textBoxPassword.Width = panelEncrypt.Width - panelSidebarEncrypt.Width - pictureBoxPassStrengthMeter.Width - 48;
      textBoxPassword.Top = panelEncrypt.Height / 2 - textBoxPassword.Height / 2;

      //「パスワード」ラベル
      labelPassword.Left = textBoxPassword.Left;
      labelPassword.Top = textBoxPassword.Top - labelPassword.Height - 4;

      // パスワード強度メーター
      pictureBoxPassStrengthMeter.Left = textBoxPassword.Right + 8;
      pictureBoxPassStrengthMeter.Top = textBoxPassword.Top;
      // パスワード強度メーターコメント
      ...
      labelPasswordStrength.Top = pictureBoxPassStrengthMeter.Top - labelPasswordStrength.Height - 8;

      // 元データを削除するかどうかのチェックボックス
      checkBoxDeleteOriginalFileAfterEncryption.Left = textBoxPassword.Left;
      checkBoxDeleteOriginalFileAfterEncryption.Top = textBoxPassword.Bottom + 4;

      // キャンセルボタン
      buttonEncryptCancel.Top = textBoxPassword.Bottom + 28;
      buttonEncryptCancel.Left = textBoxPassword.Right - buttonEncryptCancel.Width;

      // O.K.ボタン
      buttonEncryptionPasswordOk.Top = buttonEncryptCancel.Top;
      buttonEncryptionPasswordOk.Left = buttonEncryptCancel.Left - buttonEncryptCancel.Width - 12;

      // 戻るボタン
      pictureBoxEncryptBackToMain.Left = panelEncrypt.Width - pictureBoxEncryptBackToMain.Width - 24;
      pictureBoxEncryptBackToMain.Top = 10;
      
    }

    private void LayoutPanelEncryptConfirm()
    {
      // 左サイドバー
      pictureBoxEncryptionConfirm.Left = 0;
      pictureBoxEncryptionConfirm.Top = panelSidebarEncryptConfirm.Height / 2 - pictureBoxEncryptionConfirm.Height / 2;
      labelEncryptionConfirm.Left = 0;
      labelEncryptionConfirm.Top = pictureBoxEncryptionConfirm.Bottom + 2;

      // レイアウトの基準（パスワード入力テキストボックス）
      textBoxRePassword.Left = panelSidebarEncrypt.Width + 8;
      textBoxRePassword.Width = panelEncryptConfirm.Width - panelSidebarEncryptConfirm.Width - pictureBoxPassStrengthMeterConfirm.Width - 48;
      textBoxRePassword.Top = panelEncryptConfirm.Height / 2 - textBoxRePassword.Height / 2;

      // パスワードチェックマーク
      pictureBoxCheckPasswordValidation.Left = textBoxRePassword.Left - pictureBoxCheckPasswordValidation.Width- 2;
      pictureBoxCheckPasswordValidation.Top = textBoxRePassword.Top;
      
      //「パスワード」ラベル
      labelInputPasswordAgain.Left = textBoxRePassword.Left;
      labelInputPasswordAgain.Top = textBoxRePassword.Top - textBoxRePassword.Height - 4;

      // パスワード強度メーター（確認用）
      pictureBoxPassStrengthMeterConfirm.Left = textBoxRePassword.Left + textBoxRePassword.Width + 8;
      pictureBoxPassStrengthMeterConfirm.Top = textBoxRePassword.Top;
      //pictureBoxPassStrengthMeterConfirm.Visible = pictureBoxPassStrengthMeter.Visible;
      ...
      pictureBoxPassStrengthMeterConfirm.Image = pictureBoxPassStrengthMeter.Image;
      // パスワード強度メーターコメント
      labelPasswordStrengthConfirm.Left = pictureBoxPassStrengthMeterConfirm.Left + pictureBoxPassStrengthMeterConfirm.Width / 2 - labelPasswordStrengthConfirm.Width / 2;
      labelPasswordStrengthConfirm.Top = pictureBoxPassStrengthMeterConfirm.Top - labelPasswordStrengthConfirm.Height - 8;
      labelPasswordStrengthConfirm.Text = labelPasswordStrength.Text;

      // 元データを削除するかどうかのチェックボックス
      checkBoxReDeleteOriginalFileAfterEncryption.Left = textBoxRePassword.Left;
      checkBoxReDeleteOriginalFileAfterEncryption.Top = textBoxRePassword.Bottom + 4;

      // キャンセルボタン
      buttonEncryptionConfirmCancel.Top = textBoxRePassword.Bottom + 28;
      buttonEncryptionConfirmCancel.Left = textBoxRePassword.Right - buttonEncryptionConfirmCancel.Width;

      // 暗号化スタートボタン
      buttonEncryptStart.Top = buttonEncryptionConfirmCancel.Top;
      buttonEncryptStart.Left = buttonEncryptionConfirmCancel.Left - buttonEncryptionConfirmCancel.Width - 12;

      // 戻るボタン
      pictureBoxEncryptConfirmBackButton.Left = panelEncryptConfirm.Width - pictureBoxEncryptConfirmBackButton.Width - 24;
      pictureBoxEncryptConfirmBackButton.Top = 10;

    }

    private void LayoutPanelDecrypt()
    {
      // 左サイドバー
      pictureBoxDecryption.Left = 0;
      pictureBoxDecryption.Top = panelSidebarDecrypt.Height / 2 - pictureBoxDecryption.Height / 2;
      labelDecryption.Left = 0;
      labelDecryption.Top = pictureBoxDecryption.Bottom + 2;

      // レイアウトの基準（パスワード入力テキストボックス）
      textBoxDecryptPassword.Left = panelSidebarEncrypt.Width + 16;
      textBoxDecryptPassword.Width = panelDecrypt.Width - panelSidebarDecrypt.Width - 32;
      textBoxDecryptPassword.Top = panelDecrypt.Height / 2 - textBoxDecryptPassword.Height / 2;
      
      //「パスワード」ラベル
      labelDecryptionPassword.Left = textBoxDecryptPassword.Left;
      labelDecryptionPassword.Top = textBoxDecryptPassword.Top - labelDecryptionPassword.Height - 4;

      // 元データを削除するかどうかのチェックボックス
      checkBoxDeleteAtcFileAfterDecryption.Left = textBoxDecryptPassword.Left;
      checkBoxDeleteAtcFileAfterDecryption.Top = textBoxDecryptPassword.Bottom + 4;
      
      // キャンセルボタン
      buttonDecryptCancel.Top = textBoxDecryptPassword.Bottom + 28;
      buttonDecryptCancel.Left = textBoxDecryptPassword.Right - buttonDecryptCancel.Width;

      // 復号スタートボタン
      buttonDecryptStart.Top = buttonDecryptCancel.Top;
      buttonDecryptStart.Left = buttonDecryptCancel.Left - buttonDecryptCancel.Width - 12;

      // 戻るボタン
      pictureBoxDecryptBackButton.Left = panelDecrypt.Width - pictureBoxDecryptBackButton.Width - 24;
      pictureBoxDecryptBackButton.Top = 10;

    }

    private void LayoutPanelRsa()
    {
      // 左サイドバー
      pictureBoxRsaPage.Left = 0;
      pictureBoxRsaPage.Top = panelHeaderRsa.Height / 2 - pictureBoxRsaPage.Height / 2;
      labelRsaDetail.Left = 0;
      labelRsaDetail.Top = pictureBoxRsaPage.Bottom + 2;

      // 中央のメッセージ（RSAの説明文） — サイドバー幅を考慮
      int contentLeft = panelHeaderRsa.Width + 16;
      int contentWidth = panelRsa.Width - panelHeaderRsa.Width - 32;
      labelRsaMessage.Left = contentLeft;
      labelRsaMessage.Width = contentWidth;
      labelRsaMessage.Height = panelRsa.Height / 2;
      labelRsaMessage.Top = panelRsa.Height / 2 - labelRsaMessage.Height / 2;

      // キャンセルボタン
      buttonRsaCancel.Left = panelRsa.Width - buttonRsaCancel.Width - 24;
      buttonRsaCancel.Top = panelRsa.Height - buttonRsaCancel.Height - 24;

      // 鍵生成ボタン
      buttonGenerateKey.Top = buttonRsaCancel.Top;
      buttonGenerateKey.Left = buttonRsaCancel.Left - buttonGenerateKey.Width - 12;

      // 戻るボタン
      pictureBoxRsaBackButton.Left = panelRsa.Width - pictureBoxRsaBackButton.Width - 24;
      pictureBoxRsaBackButton.Top = 10;
    }

    private void LayoutPanelRsaKey()
    {
      // 左サイドバー
      pictureBoxRsaType.Left = 0;
      pictureBoxRsaType.Top = panelHeaderRsaKey.Height / 2 - pictureBoxRsaType.Height / 2;
      labelRsaKeyName.Left = 0;
      labelRsaKeyName.Top = pictureBoxRsaType.Bottom + 2;

      // レイアウトの基準（パスワード入力テキストボックス）
      textBoxHashString.Left = panelHeaderRsaKey.Width + 8;
      textBoxHashString.Width = panelRsaKey.Width - panelHeaderRsaKey.Width - 32;
      textBoxHashString.Top = panelRsaKey.Height / 2 - textBoxHashString.Height / 2;

      //「ハッシュ値」ラベル
      comboBoxHashList.Left = textBoxHashString.Left;
      comboBoxHashList.Top = textBoxHashString.Top - comboBoxHashList.Height - 4;

      // RSAファイルの「ドラッグ＆ドロップ」を促すメッセージラベル
      labelRsaDescription.Left = textBoxHashString.Left;
      labelRsaDescription.Width = textBoxHashString.Width;
      labelRsaDescription.Height = comboBoxHashList.Top - 8;
      labelRsaDescription.Top = comboBoxHashList.Top - labelRsaDescription.Height - 4;

      // インフォ画像アイコン
      pictureBoxInfoBalloon.Left = textBoxHashString.Left;
      pictureBoxInfoBalloon.Top = textBoxHashString.Bottom + 4;
      // RSAの説明ラベル
      labelRsaInformation.Left = textBoxHashString.Left + pictureBoxInfoBalloon.Width + 4;
      labelRsaInformation.Top = pictureBoxInfoBalloon.Top + 4;
      labelRsaInformation.Width = textBoxHashString.Width - pictureBoxInfoBalloon.Width;
      
      // キャンセルボタン
      buttonRsaKeyCancel.Left = panelRsaKey.Width - buttonRsaKeyCancel.Width - 24;
      buttonRsaKeyCancel.Top = panelRsaKey.Height - buttonRsaKeyCancel.Height - 24;

      // 戻るボタン
      pictureBoxRsaKeyBackButton.Left = panelRsaKey.Width - pictureBoxRsaKeyBackButton.Width - 24;
      pictureBoxRsaKeyBackButton.Top = 10;

    }

    private void LayoutPanelPostPpapRegister()
    {
      // 左サイドバー
      pictureBoxPostPpapPage.Left = 0;
      pictureBoxPostPpapPage.Top = panelHeaderPostPpapRegister.Height / 2 - pictureBoxPostPpapPage.Height / 2;
      labelPostPpapPageTitle.Left = 0;
      labelPostPpapPageTitle.Top = pictureBoxPostPpapPage.Bottom + 2;

      // レイアウトの基準（あなたのメールアドレス）
      textBoxYourMailAddress.Left = panelHeaderPostPpapRegister.Width + 8;
      textBoxYourMailAddress.Top = panelPostPpapRegister.Height / 2 + 24;
      textBoxYourMailAddress.Width = panelPostPpapRegister.Width - panelHeaderPostPpapRegister.Width - 24;

      //「あなたのメールアドレス」ラベル
      labelYourMailAddress.Left = textBoxYourMailAddress.Left;
      labelYourMailAddress.Top = textBoxYourMailAddress.Top - labelYourMailAddress.Height - 4;

      //「登録する」ボタン
      buttonRegisterYourMailAddress.Left = textBoxYourMailAddress.Right - buttonRegisterYourMailAddress.Width;
      buttonRegisterYourMailAddress.Top = textBoxYourMailAddress.Bottom + 8;

      // 登録中のプログレスバー詳細ラベル
      labelPostPpapStatus.Left = textBoxYourMailAddress.Left;
      labelPostPpapStatus.Top = buttonRegisterYourMailAddress.Bottom + 16;
      labelPostPpapStatus.Width = textBoxYourMailAddress.Width;

      // 登録中のプログレスバー
      progressBarPostPpap.Left = labelPostPpapStatus.Left;
      progressBarPostPpap.Top = labelPostPpapStatus.Bottom + 4;
      progressBarPostPpap.Width = labelPostPpapStatus.Width;
      
      // PPAPメールアドレス登録の説明ラベル
      labelPpapDescription.Left = textBoxYourMailAddress.Left;
      labelPpapDescription.Top = labelYourMailAddress.Top - labelPpapDescription.Height - 16;

      // 戻るボタン
      pictureBoxPostPpapRegisterBackButton.Left = panelPostPpapRegister.Width - pictureBoxPostPpapRegisterBackButton.Width - 24;
      pictureBoxPostPpapRegisterBackButton.Top = 10;

    }

    private void LayoutPanelPostPpapMain()
    {
      // 左サイドバー
      pictureBoxPostPpapMain.Left = 0;
      pictureBoxPostPpapMain.Top = panelHeaderPostPpapMain.Height / 2 - pictureBoxPostPpapMain.Height / 2;
      labelPostPpapMain.Left = 0;
      labelPostPpapMain.Top = pictureBoxPostPpapMain.Top + pictureBoxPostPpapMain.Height + 2;

      // 基準となる宛先のメールアドレスのテキストボックス
      textBoxPuclicRecipientMailAddress.Left = panelHeaderPostPpapMain.Width + 8;
      textBoxPuclicRecipientMailAddress.Top = panelPostPpapMain.Height / 4 - textBoxPuclicRecipientMailAddress.Height / 2;
      textBoxPuclicRecipientMailAddress.Width = panelPostPpapMain.Width - panelHeaderPostPpapMain.Width - 24;
      
      //「検索」ボタン
      buttonPuclicMailAddressSearch.Left = textBoxPuclicRecipientMailAddress.Right - buttonPuclicMailAddressSearch.Width;
      buttonPuclicMailAddressSearch.Top = textBoxPuclicRecipientMailAddress.Bottom + 4;

      //「宛先のメールアドレス」ラベル
      labelRecipientMailAddress.Left = textBoxPuclicRecipientMailAddress.Left;
      labelRecipientMailAddress.Top = textBoxPuclicRecipientMailAddress.Top - labelRecipientMailAddress.Height - 4;

      // 宛先のメールアドレスが有効か無効かのアイコン
      pictureBoxValid.Left = textBoxPuclicRecipientMailAddress.Left;
      pictureBoxValid.Top = textBoxPuclicRecipientMailAddress.Bottom + 8;
      // 宛先のメールアドレスの有効・無効の詳細メッセージ
      labelValidMessage.Left = pictureBoxValid.Left + pictureBoxValid.Width + 4;
      labelValidMessage.Top = pictureBoxValid.Top;

      // 宛先のメールアドレスの有効・無効の詳細アイコン
      pictureBoxValidDetails.Left = textBoxPuclicRecipientMailAddress.Left;
      pictureBoxValidDetails.Top = pictureBoxValid.Bottom + 4;
      // 宛先のメールアドレスの有効・無効の詳細メッセージ（さらに詳しい内容）
      labelValidDetails.Left = pictureBoxValidDetails.Right + 4;
      labelValidDetails.Top = pictureBoxValidDetails.Top;

      // 自身のメールアドレスアイコン
      pictureBoxMailAddress.Left = textBoxPuclicRecipientMailAddress.Left;
      pictureBoxMailAddress.Top = 16;
      // 自身のメールアドレス
      labelRegisteredMailAddress.Left = pictureBoxMailAddress.Right + 4;
      labelRegisteredMailAddress.Top = pictureBoxMailAddress.Top + pictureBoxMailAddress.Height / 2 - labelRegisteredMailAddress.Height / 2;

      // ドラッグ＆ドロップエリア
      panelPublicDragAndDrop.Left = textBoxPuclicRecipientMailAddress.Left;
      panelPublicDragAndDrop.Top = pictureBoxValidDetails.Bottom + 24;
      panelPublicDragAndDrop.Width = textBoxPuclicRecipientMailAddress.Width;
      panelPublicDragAndDrop.Height = panelPostPpapMain.Height / 4 * 2;

      // ドラッグ＆ドロップ内のメッセージラベル
      labelPasswordSharingIfRegistered.Width = (int)(panelPublicDragAndDrop.Width * 0.8);
      labelPasswordSharingIfRegistered.Height = (int)(panelPublicDragAndDrop.Height * 0.6);
      labelPasswordSharingIfRegistered.Left = panelPublicDragAndDrop.Height / 2 - labelPasswordSharingIfRegistered.Height / 2;
      labelPasswordSharingIfRegistered.Top = panelPublicDragAndDrop.Width / 2 - labelPasswordSharingIfRegistered.Width / 2;

      // 戻るボタン
      pictureBoxPostPpapBackButton.Left = panelPostPpapMain.Width - pictureBoxPostPpapBackButton.Width - 24;
      pictureBoxPostPpapBackButton.Top = 10;
      
    }

    // Layout
    private void LayoutPanelProgressState()
    {
      // 左サイドバー
      pictureBoxProgress.Left = 0;
      pictureBoxProgress.Top = panelSidebarProgress.Height / 2 - pictureBoxProgress.Height / 2;
      labelProgress.Left = 0;
      labelProgress.Top = pictureBoxProgress.Top + pictureBoxProgress.Height + 2;

      // 基準となるプログレスバー
      progressBar.Left = panelSidebarProgress.Width + 16;
      progressBar.Top = panelProgressState.Height / 2 - progressBar.Height / 2;
      progressBar.Width = panelProgressState.Width - panelSidebarProgress.Width - 32;

      // プログレスバー下のパーセンテージラベル
      labelProgressPercentText.Left = progressBar.Left;
      labelProgressPercentText.Top = progressBar.Top + progressBar.Height + 2;
      labelProgressPercentText.Width = progressBar.Width;

      // プログレスバー上にある詳細ラベル（AutoSize = true）
      labelProgressMessageText.Left = progressBar.Left;
      labelProgressMessageText.Top = progressBar.Top - labelProgressMessageText.Height - 2;

      // プログレスバー上にある処理内容ラベル（AutoSize = true）
      labelCryptionType.Left = progressBar.Left;
      labelCryptionType.Top = progressBar.Top - labelProgressMessageText.Height - labelCryptionType.Height - 4;

      // キャンセルボタン
      buttonCancel.Left = progressBar.Left + progressBar.Width - buttonDecryptCancel.Width;
      buttonCancel.Top = progressBar.Top + progressBar.Height + 28;

      // 戻るボタン
      pictureBoxProgressStateBackButton.Left = panelProgressState.Width - pictureBoxProgressStateBackButton.Width - 24;
      pictureBoxProgressStateBackButton.Top = 10;


    }


    private void panelPublicDragAndDrop_Paint(object sender, PaintEventArgs e)
    {
      var p = (Panel)sender;
      ...
      // テーマに応じて点線の色を切り替え
      var penColor = _isDarkTheme ? Color.White : Color.Black;
      using Pen dashedPen = new Pen(penColor, 2);
      // 点線のパターンを指定
      dashedPen.DashStyle = DashStyle.Dash;

      // 必要に応じて、より細かいドットにするなら DashStyle.Dot
      // カスタムパターンなら dashedPen.DashPattern = new float[] { 5, 2 }; // 5塗って2空ける

      // Panelの内側に収まるように矩形を計算（線の太さの半分だけ内側を狙うときれいです）
      var thickness = (int)dashedPen.Width;
      var rect = new Rectangle(
        thickness / 2,
        thickness / 2,
        p.ClientSize.Width - thickness,
        p.ClientSize.Height - thickness
      );
    ...
    private void Form1_FormClosed(object sender, FormClosedEventArgs e)
    {
      ...
      if (AppSettings.Instance.SettingSource == AppSettings.SettingDataLocation.IniFile)
      {
        AppSettings.Instance.WriteOptionToIniFile(AppSettings.Instance.IniFilePath);

      }
      else if (AppSettings.Instance.SettingSource == AppSettings.SettingDataLocation.Registry)
      {
      ...
      {
        // なにも設定保存せず終了する
        // AppSettings.SettingDataLocation.None
      }
    }
    ...
    {
      var FormThemeColor = _isDarkTheme ? "dark" : "light";
    ...
    {
      // Post-PPAP メイン画面で未検索の場合はドラッグ＆ドロップを無効にする
      if (panelPostPpapMain.Visible && !_postPpapSearched)
      {
      ...
      else if (panelPostPpapMain.Visible && _postPpapSearched)
      {
        // 検索済みの場合はドラッグ＆ドロップを受け付ける
      }
      else
      {
        e.Effect = DragDropEffects.None;
      ...
      if (e.Data.GetDataPresent(DataFormats.FileDrop) == true)
      {
        ...
        // Post-PPAP のドラッグ＆ドロップエリアにカラー変更
        if (panelPostPpapMain.Visible && _postPpapSearched && _foundRecipientKey != null)
        {
          if (!_isDarkTheme)
            panelPublicDragAndDrop.BackColor = LightDragColor;
          else
            panelPublicDragAndDrop.BackColor = DarkDragColor;
        }

        if (!_isDarkTheme)
        { // light theme
          panelStartPage.BackColor = LightDragColor;
        }
        else
        { // dark theme
          panelStartPage.BackColor = DarkDragColor;
        }
      }
    ...
    private void Form1_DragLeave(object sender, EventArgs e)
    {
      if (!_isDarkTheme)
      { // light theme
        panelStartPage.BackColor = LightContentColor;
        panelPublicDragAndDrop.BackColor = Color.Transparent;
      }
      else
      { // dark theme
    ...
    private void Form1_DragDrop(object sender, DragEventArgs e)
    {
      ...
      if (panelPostPpapMain.Visible && _postPpapSearched)
      {
        ...
        if (_foundRecipientKey != null)
        {
          // 公開鍵が見つかっている → 公開鍵暗号化処理へ
          StartPostPpapEncryption();
        }
        else
        {
          // 公開鍵が見つからなかった → パスワード入力画面へ
          panelPostPpapMain.Visible = false;
    ...
    private bool DeleteData(List<string> FileList)
    {
      ...
      pictureBoxProgress.Image = ThemeIconManager.Get(_iconDecrypt, ThemeIconManager.IconState.Normal);
    ...
    private void CustomDialogMessageShow(int fileType, string filePath, IReadOnlyList<string> md5FileHash = null)
    {
      ...
      switch (fileType)
      {
        ...
        case 1:
          ...
          else if (decryption5 != null)
          {
            TempOverWriteOption = decryption5.TempOverWriteOption;
          }
          else
          {
               ...
               + Environment.NewLine + filePath))
          {
            ...
            {
              decryption5.TempOverWriteOption = TempOverWriteOption;
            }
            else
            {
        ...
        case 2:
                 ...
                 )
          {
            ...
            {
              if (decryption5 != null)
                decryption5.TempHashMismatchContinueOption = CONTINUE;
              else if (decryption4 != null)
            ...
            else
            {
              ...
              if (decryption5 != null)
                decryption5.TempHashMismatchContinueOption = USER_CANCELED;
              else if (decryption4 != null)
    ...
    private void backgroundWorker_Encryption_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
      ...
      if (e.Cancelled)
      {
        ...
        // Atc file is deleted
        if (File.Exists(encryption5.AtcFilePath) == true)
        {
          FileSystem.DeleteFile(encryption5.AtcFilePath);
        }
        ...
        switch (encryption5.ReturnCode)
        {
          ...
          case NO_DISK_SPACE:
            ...
            MessageBox.Show(this,
              Resources.DialogMessageNoDiskSpace + Environment.NewLine + encryption5.DriveName,
          ...
          case DIRECTORY_NOT_FOUND:
            ...
            //
            MessageBox.Show(this,
              Resources.DialogMessageDirectoryNotFound + Environment.NewLine + encryption5.ErrorMessage,
              Resources.DialogTitleError, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            break;
          ...
          case DRIVE_NOT_FOUND:
            ...
            //
            MessageBox.Show(this,
              Resources.DialogMessageDriveNotFound + Environment.NewLine + encryption5.ErrorMessage,
              Resources.DialogTitleError, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            break;
          ...
          case FILE_NOT_LOADED:
            ...
            //
            MessageBox.Show(this,
              Resources.DialogMessageFileNotLoaded + Environment.NewLine + encryption5.ErrorFilePath,
              Resources.DialogTitleError, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            break;
          ...
          case FILE_NOT_FOUND:
            ...
            //
            MessageBox.Show(this,
              Resources.DialogMessageFileNotFound + Environment.NewLine + encryption5.ErrorFilePath,
              Resources.DialogTitleError, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            break;
          ...
          case IO_EXCEPTION:
            ...
            MessageBox.Show(this, encryption5.ErrorMessage,
              Resources.DialogTitleError, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            break;
    ...
    private void backgroundWorker_Decryption_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
      ...
      */
      ...
      {
        ReturnCode = decryption5.ReturnCode;
      }
      else
      {
      ...
      switch (ReturnCode)
      {
        ...
        case DECRYPT_SUCCEEDED:
          ...
          {
            OutputFileList.AddRange(decryption5.OutputFileList);
          }
          else
          {
        ...
        case NOT_ATC_DATA:
          ...
          if (decryption5 != null)
          {
            ErrorFilePath = decryption5.ErrorFilePath;
          }
          else if (decryption4 != null)
          {
        ...
        case NO_DISK_SPACE:
          ...
          if (decryption5 != null)
          {
            ErrorFilePath = decryption5.DriveName;
          }
          else if (decryption4 != null)
          {
    ...
    {
      //--------------------------------------------------------------
      // RSA コマンドライン処理: 鍵ファイルを読み込み既存フローに合流
      //--------------------------------------------------------------
      // /genrsa: 鍵ペア生成（暗号化/復号なし）
      if (!string.IsNullOrEmpty(AppSettings.Instance.GenerateRsaKeyOutputDir))
      {
        ExecuteRsaGenerateKeyFromCommandLine();
        return;
      }
      // /pubkey: 公開鍵XMLを読み込んで RSA 暗号化を即実行
      if (!string.IsNullOrEmpty(AppSettings.Instance.RsaPublicKeyFilePath))
      {
        try
        {
          XmlPublicKeyString = File.ReadAllText(AppSettings.Instance.RsaPublicKeyFilePath, Encoding.UTF8);
        }
        catch (Exception ex)
        {
          MessageBox.Show("Error reading public key:\n" + ex.Message,
            Resources.DialogTitleAlert, MessageBoxButtons.OK, MessageBoxIcon.Error);
          return;
        }
        ...
        // ファイル未指定の場合は鍵を保持して通常のUI表示（ドラッグ＆ドロップ待ち）
      }
      // /p-ppap: Post-PPAP 暗号化（キーサーバから公開鍵を検索して暗号化）
      if (!string.IsNullOrEmpty(AppSettings.Instance.PostPpapRecipientEmail))
      {
        if (AppSettings.Instance.FileList.Any())
        {
          ExecutePostPpapEncryptFromCommandLine();
          return;
        }
      }

      // /privkey: 秘密鍵XMLを読み込んで RSA 復号を即実行
      if (!string.IsNullOrEmpty(AppSettings.Instance.RsaPrivateKeyFilePath))
      {
        try
        {
          XmlPrivateKeyString = File.ReadAllText(AppSettings.Instance.RsaPrivateKeyFilePath, Encoding.UTF8);
        }
        catch (Exception ex)
        {
      ...
      else
      {
        ...
        if (AppSettings.Instance.FileList.Any())
        {
          ...
          else if (ProcessType == PROCESS_TYPE_DECRYPTION)
          {
            ...
            // 公開鍵暗号ファイルかどうか事前判定
            if (TryAutoDecryptWithPrivateKey())
            {
              // 秘密鍵で自動復号を開始
              return;
            }
          ...
          else if (ProcessType == PROCESS_TYPE_RSA_ENCRYPTION || ProcessType == PROCESS_TYPE_RSA_DECRYPTION)
          {
            for (var i = AppSettings.Instance.FileList.Count - 1; i >= 0; i--)
            {
              ...
              if (AppSettings.Instance.CheckFileType(AppSettings.Instance.FileList[i]) == 5)
              {
                ...
                // RSA-4096 以外の鍵は拒否する
                var tempKey = CryptoHelper5.ParseRsaPublicKeyFromXml(xmlContent);
                if (tempKey.Modulus.BitLength < 4096)
                {
                  labelRsaMessage.Text = Resources.LabelRsaKeyTooSmall;
                  break;
                }

                XmlPublicKeyString = xmlContent;
    ...
    private void panelStartPage_VisibleChanged(object sender, EventArgs e)
    {
      ...
      BeginInvoke(new Action(() => LayoutActivePanel()));
    ...
    private void panelEncrypt_VisibleChanged(object sender, EventArgs e)
    {
      ...
      {
        pictureBoxEncryption.Image = ThemeIconManager.Get(_iconEncrypt, ThemeIconManager.IconState.Normal);
        labelEncryption.Text = labelAtc.Text;
      }
      else if (AppSettings.Instance.EncryptionFileType == FILE_TYPE_ATC_EXE)
      {
        pictureBoxEncryption.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Normal);
        labelEncryption.Text = labelRsa.Text;
      }
      else
      {
        pictureBoxEncryption.Image = ThemeIconManager.Get(_iconEncrypt, ThemeIconManager.IconState.Normal);
    ...
    private void panelEncryptConfirm_VisibleChanged(object sender, EventArgs e)
    {
      ...
      // パスワードマスクするか
      if (AppSettings.Instance.fNotMaskPassword)
      {
        textBoxRePassword.IsPasswordVisible = true;
      }
      else
      {
        textBoxRePassword.IsPasswordVisible = false;
      }

      // 暗号化後に元のファイルを削除するか
      if (AppSettings.Instance.fDelOrgFile)
      {
        checkBoxReDeleteOriginalFileAfterEncryption.Checked = true;
      }
      else
    ...
    private void textBoxRePassword_TextChanged(object sender, EventArgs e)
    {
      ...
      else
      {
        ...
        if (textBoxPassword.Text == textBoxRePassword.Text)
        {
          pictureBoxCheckPasswordValidation.Visible = true;
          pictureBoxCheckPasswordValidation.Image = pictureBoxValidIcon.Image;
          textBoxRePassword.BackColor = Color.Honeydew;
        }
        else
        {
          if (textBoxRePassword.Text.Length == 0)
          {
            pictureBoxCheckPasswordValidation.Visible = false;
            textBoxRePassword.BackColor = Color.White;
          }
          else
          {
    ...
    private void textBoxRePassword_KeyDown(object sender, KeyEventArgs e)
    {
      ...
      else if (e.KeyCode == Keys.Back && textBoxRePassword.Text.Length == 0)
      {
        e.Handled = true;
        e.SuppressKeyPress = true;
        // 前のパスワード入力画面へ戻る
        buttonEncryptionConfirmCancel_Click(sender, e);
      }
    }
    ...
    private void panelDecrypt_VisibleChanged(object sender, EventArgs e)
    {
      ...
      // Not mask password character
      AppSettings.Instance.fNotMaskPassword = textBoxDecryptPassword.Visible;
    ...
    {
      if (!panelProgressState.Visible) return;
      BeginInvoke(new Action(() => LayoutActivePanel()));
    ...
    ///// </summary>
    private void textBoxPassword_IsPasswordVisibleChanged(object sender, EventArgs e)
    {
      if (textBoxPassword.IsPasswordVisible == true)
      {
        // textBoxRePassword.IsPasswordVisible = true;
    ...
    {
      pictureBoxEncryptConfirmBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Hover);
    }

    private void pictureBoxEncryptConfirmBackButton_MouseLeave(object sender, EventArgs e)
    {
      pictureBoxEncryptConfirmBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
    }
    ...
    {
      //Not mask password character
      AppSettings.Instance.fNotMaskPassword = textBoxPassword.IsPasswordVisible;
    ...
    private void EncryptionProcess()
    {
      ...
      panelProgressState.Visible = true;
      labelCryptionType.Text = Resources.LabelProcessNameEncrypt;
      pictureBoxProgress.Image = ThemeIconManager.Get(_iconEncrypt, ThemeIconManager.IconState.Normal);
      ...
      {
        pictureBoxProgress.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Normal);
        labelProgress.Text = labelRsa.Text;
      ...
      if (AppSettings.Instance.fAllFilePack == true)
      {
            ...
            AppSettings.Instance.EncryptionFileType == FILE_TYPE_ATC_EXE)
        {
          ...
          encryption5.MissTypeLimits = input_limit;
        }
        ...
        if (AppSettings.Instance.EncryptionFileType == FILE_TYPE_ATC_EXE)
        {
          encryption5.fExecutable = true;

          Int64 ExeOutSize = 0;
          if (AppSettings.Instance.fOutputExeForOlderOS == true)
          {
            encryption5.ExeToolVersionString = "4.0";
            ExeOutSize = FileEncrypt5.ExeOutFileSize[0];
          }
          else
          {
        ...
        if (string.IsNullOrEmpty(XmlPublicKeyString) == false)
        {
          encryption5.AddRecipient(CryptoHelper5.ParseRsaPublicKeyFromXml(XmlPublicKeyString));
        }

        // Post-PPAP: 検索で見つかった受信者の公開鍵を追加（ハイブリッド暗号のみ）
        if (_foundRecipientKey != null)
        {
          var rsaPub = CryptoHelper5.ParseRsaPublicKeyFromBase64(
              _foundRecipientKey.Modulus, _foundRecipientKey.Exponent);
          var mlkemPubBytes = Convert.FromBase64String(_foundRecipientKey.MlKemPublicKey);
          var mlkemPub = Org.BouncyCastle.Crypto.Parameters.MLKemPublicKeyParameters.FromEncoding(
              Org.BouncyCastle.Crypto.Parameters.MLKemParameters.ml_kem_768, mlkemPubBytes);
          encryption5.AddRecipient(rsaPub, mlkemPub, _foundRecipientKey.AlgorithmId);
        }
        ...
        if (AppSettings.Instance.EncryptionFileType == FILE_TYPE_NONE ||
            ...
            AppSettings.Instance.EncryptionFileType == FILE_TYPE_ATC_EXE)
        {
          encryption5.fKeepTimeStamp = AppSettings.Instance.fKeepTimeStamp;
        }
    ...
    {
      pictureBoxEncryptBackToMain.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Hover);
    }

    private void pictureBoxEncryptBackButton_MouseLeave(object sender, EventArgs e)
    {
      pictureBoxEncryptBackToMain.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
    }
    ...
    {
      if (textBoxDecryptPassword.Visible == true)
      {
    ...
    {
      // Enter キーを入力キーとして認識させる（KeyDown で処理するため）
      if (e.KeyCode == Keys.Enter)
      {
        e.IsInputKey = true;
      }
    ...
    private void textBoxDecryptPassword_KeyDown(object sender, KeyEventArgs e)
    {
      if (e.KeyCode == Keys.Enter)
      {
        e.Handled = true;
        e.SuppressKeyPress = true;
        buttonDecryptStart_Click(sender, e);
      }
    }
    ...
    //======================================================================
    /// <summary>
    /// 公開鍵暗号ファイルかどうかを判定し、自身の秘密鍵で自動復号を開始する。
    /// 公開鍵暗号ファイルかつ秘密鍵が利用可能な場合は true を返す。
    /// </summary>
    //======================================================================
    private bool TryAutoDecryptWithPrivateKey()
    {
      if (AppSettings.Instance.FileList.Count == 0) return false;

      // 最初のファイルのヘッダを読み取って公開鍵暗号かどうか確認
      FileDecrypt5 testDecrypt;
      try
      {
        testDecrypt = new FileDecrypt5(AppSettings.Instance.FileList[0]);
        if (!testDecrypt.fPublicKeyEncryption) return false;
      }
      catch
      {
        return false;
      ...
      // Post-PPAP 全アカウントの秘密鍵を試し、フィンガープリントが一致するものを探す
      var accounts = PostPpapManager.LoadAccounts();
      if (accounts.Count == 0)
      {
        // XML 秘密鍵がロード済みなら、そちらで復号を試みる
        if (!string.IsNullOrEmpty(XmlPrivateKeyString))
        {
          try
          {
            var xmlKey = CryptoHelper5.ParseRsaPrivateKeyFromXml(XmlPrivateKeyString);
            if (testDecrypt.HasMatchingRecipient(xmlKey))
            {
              // XML 秘密鍵で復号（_postPpapPrivateKey は null のまま、既存パスで処理）
              FileIndex = 0;
              OutputFileList = new List<string>();
              DecryptionProcess();
              return true;
      ...
      foreach (var account in accounts)
      {
        try
        {
          using (var rsaCsp = PostPpapManager.LoadPrivateKey(account))
          {
            var bcKey = CryptoHelper5.ParseRsaPrivateKeyFromCsp(rsaCsp);

            // ML-KEM 秘密鍵のロード（ハイブリッドアカウントの場合）
            Org.BouncyCastle.Crypto.Parameters.MLKemPrivateKeyParameters mlkemKey = null;
            if (!string.IsNullOrEmpty(account.MlKemEncryptedPrivateKey))
            {
              try { mlkemKey = PostPpapManager.LoadMlKemPrivateKey(account); } catch { }
            }

            System.Diagnostics.Debug.WriteLine(
                $"[PostPPAP] Email: {account.Email}, AlgorithmId: 0x{account.AlgorithmId:X2}, MlKem: {mlkemKey != null}");
            System.Diagnostics.Debug.WriteLine(
                $"[PostPPAP] ファイル EncryptionMode: {testDecrypt.EncryptionMode}, PublicKeyAlgorithm: 0x{testDecrypt.PublicKeyAlgorithm:X2}");

            if (testDecrypt.HasMatchingRecipient(bcKey, mlkemKey))
            {
              System.Diagnostics.Debug.WriteLine("[PostPPAP] フィンガープリント一致 → 自動復号開始");
              _postPpapPrivateKey = bcKey;
              _postPpapMlKemPrivateKey = mlkemKey;
              FileIndex = 0;
              OutputFileList = new List<string>();
              DecryptionProcess();
              return true;
            ...
            {
              System.Diagnostics.Debug.WriteLine("[PostPPAP] フィンガープリント不一致");
            }
          }
        }
        catch
      ...
      // どのアカウントのフィンガープリントも一致しない → パスワード画面にフォールバック
      return false;
    }
    ...
    private void DecryptionProcess()
    {
      ...
      //-----------------------------------
      // Ver.5 (150 <= version < 160)
      else if (decryption4.DataFileVersion < 160)
      {
        decryption2 = null;
        decryption3 = null;
        decryption4 = null;
        decryption5 = new FileDecrypt5(AtcFilePath);
        decryption5.fNoParentFolder = AppSettings.Instance.fNoParentFldr;
        decryption5.NumberOfFiles = FileIndex + 1;
        decryption5.TotalNumberOfFiles = AppSettings.Instance.FileList.Count;
        decryption5.fSameTimeStamp = AppSettings.Instance.fSameTimeStamp;
        if (_postPpapPrivateKey != null)
        {
          decryption5.SetPrivateKey(_postPpapPrivateKey);
          if (_postPpapMlKemPrivateKey != null)
            decryption5.SetMlKemPrivateKey(_postPpapMlKemPrivateKey);
        }
        else if (string.IsNullOrEmpty(XmlPrivateKeyString) == false)
        {
          decryption5.SetPrivateKey(CryptoHelper5.ParseRsaPrivateKeyFromXml(XmlPrivateKeyString));
        }
        decryption5.TempOverWriteOption = (AppSettings.Instance.fDecryptConfirmOverwrite == false ? OVERWRITE_ALL : 0);
        TempOverWriteOption = decryption5.TempOverWriteOption;
        if (LimitOfInputPassword == -1)
        {
    ...
    {
      // Post-PPAP 秘密鍵をクリア
      _postPpapPrivateKey = null;
      _postPpapMlKemPrivateKey = null;
    ...
    {
      pictureBoxDecryptBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Hover);
    }

    private void pictureBoxDecryptBackButton_MouseLeave(object sender, EventArgs e)
    {
      pictureBoxDecryptBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
    }
    ...
    {
      pictureBoxProgressStateBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Hover);
    }

    private void pictureBoxProgressStateBackButton_MouseLeave(object sender, EventArgs e)
    {
      pictureBoxProgressStateBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
    }
    ...
    private void panelStartPage_MouseEnter(object sender, EventArgs e)
    {
      if (!_isDarkTheme)
      { // light theme
        panelStartPage.BackColor = LightHoverColor;
      }
      else
      { // dark theme
        panelStartPage.BackColor = DarkHoverColor;
      }
    }
    ...
    private void pictureBoxAtc_MouseEnter(object sender, EventArgs e)
    {
      if (!_atcChecked)
      {
        pictureBoxAtc.Image = ThemeIconManager.Get(_iconEncrypt, ThemeIconManager.IconState.Hover);
      }
    }
    ...
    private void pictureBoxAtc_MouseLeave(object sender, EventArgs e)
    {
      if (!_atcChecked)
      {
        pictureBoxAtc.Image = ThemeIconManager.Get(_iconEncrypt, ThemeIconManager.IconState.Normal);
      }
    }
    ...
    private void pictureBoxRsa_MouseEnter(object sender, EventArgs e)
    {
      if (!_rsaChecked)
      {
        pictureBoxRsa.Image = ThemeIconManager.Get(_iconPublicKey, ThemeIconManager.IconState.Hover);
      }
    }
    ...
    private void pictureBoxRsa_MouseLeave(object sender, EventArgs e)
    {
      if (!_rsaChecked)
      {
        pictureBoxRsa.Image = ThemeIconManager.Get(_iconPublicKey, ThemeIconManager.IconState.Normal);
      }
    }
    ...
    private void pictureBoxDec_Click(object sender, EventArgs e)
    {
      ...
      {
        _decChecked = false;
        pictureBoxDec.Image = ThemeIconManager.Get(_iconDecrypt, ThemeIconManager.IconState.Normal);
      }
    }
    ...
    private void pictureBoxDec_MouseEnter(object sender, EventArgs e)
    {
      if (!_decChecked)
      {
        pictureBoxDec.Image = ThemeIconManager.Get(_iconDecrypt, ThemeIconManager.IconState.Hover);
      }
    }
    ...
    {
      // 設定画面を開く（ToolStripMenuItemSetting_Click と同じ処理）
      ToolStripMenuItemSetting_Click(sender, e);
    }

    private void pictureBoxOption_MouseEnter(object sender, EventArgs e)
    {
      pictureBoxOption.Image = ThemeIconManager.Get(_iconSettings, ThemeIconManager.IconState.Hover);
    ...
    private void pictureBoxOption_MouseLeave(object sender, EventArgs e)
    {
      pictureBoxOption.Image = ThemeIconManager.Get(_iconSettings, ThemeIconManager.IconState.Normal);
    ...
    private const int DWMWA_SYSTEMBACKDROP_TYPE = 38;  // Windows 11 22621+

    // DwmExtendFrameIntoClientArea 用 MARGINS 構造体
    ...
    // Windows 10 Acrylic 用（SetWindowCompositionAttribute）
    [StructLayout(LayoutKind.Sequential)]
    private struct AccentPolicy
    {
      public int AccentState;
      public int AccentFlags;
      public int GradientColor; // AABBGGRR 形式
    ...
    private delegate int SetWindowCompositionAttributeDelegate(IntPtr hwnd, ref WindowCompositionAttributeData data);

    // Acrylic/Mica バックドロップが有効かどうか
    private bool _backdropEnabled = false;
    ...
    private void ChangeTheme(Control.ControlCollection container, bool fDark)
    {
      ...
      using (var dwmapiDll = new UnManagedDll(dllPath))
      {
        ...
        // タイトルバー色の変更（Windows 11 Build 22000 以降）
        if (Environment.OSVersion.Version.Build >= 22000)
        {
          // COLORREF: 0x00BBGGRR（リトルエンディアン BGR 形式）
          // #070f1b → R=0x07, G=0x0F, B=0x1B → COLORREF = 0x001B0F07
          var captionColor = fDark ? 0x001B0F07 : -1; // -1 (DWMWA_COLOR_DEFAULT) でシステム既定色に戻す
          DwmSetWindowAttribute(this.Handle, DWMWA_CAPTION_COLOR, ref captionColor, sizeof(int));

          var borderColor = fDark ? 0x001B0F07 : -1;
      ...
      // This form
      this.BackColor = fDark ? Color.Black : LightBaseColor;
      ...
      foreach (Control component in container)
      {
        if (component is System.Windows.Forms.Panel)
        {
          ...
          component.BackColor = fDark ? Color.Black : LightBaseColor;
          component.ForeColor = fDark ? Color.White : SystemColors.ControlText;
        }
        else if (component is Button)
        {
          component.BackColor = fDark ? Color.Black : LightContentColor;
          component.ForeColor = fDark ? Color.White : SystemColors.ControlText;
        }
        ...
        // 各タブページの左側サイドバー
        var sidebarColor = fDark ? DarkSidebarColor : LightSidebarColor;
        panelSidebarStart.BackColor = sidebarColor;
        panelSidebarEncrypt.BackColor = sidebarColor;
        panelSidebarEncryptConfirm.BackColor = sidebarColor;
        panelSidebarDecrypt.BackColor = sidebarColor;
        panelSidebarProgress.BackColor = sidebarColor;

        // 各画面のヘッダーパネル
        var headerColor = fDark ? DarkHeaderColor : LightHeaderColor;
        panelHeaderRsa.BackColor = headerColor;
        panelHeaderRsaKey.BackColor = headerColor;
        panelHeaderPostPpapRegister.BackColor = headerColor;
        panelHeaderPostPpapMain.BackColor = headerColor;

      }

      // ThemeIconManager のテーマ切り替え＋全アイコン再描画
      ThemeIconManager.CurrentTheme = fDark ? ThemeIconManager.Theme.Dark : ThemeIconManager.Theme.Light;
      RefreshAllIcons();
    ...
    /// <summary>
    /// 埋め込みリソースからベースアイコンを読み込み、ThemeIconManager を初期設定する。
    /// </summary>
    private void InitializeIcons()
    {
      _iconEncrypt = LoadEmbeddedBitmap("AttacheCase.Resources.menu_encrypt.png");
      _iconDecrypt = LoadEmbeddedBitmap("AttacheCase.Resources.menu_decrypt.png");
      _iconPublicKey = LoadEmbeddedBitmap("AttacheCase.Resources.menu_public_key.png");
      _iconBack = LoadEmbeddedBitmap("AttacheCase.Resources.menu_back.png");
      _iconSettings = LoadEmbeddedBitmap("AttacheCase.Resources.menu_settings.png");
      _iconPostPpap = LoadEmbeddedBitmap("AttacheCase.Resources.menu_post_ppap.png");

      // resx 埋め込みアイコンのベース画像を保持（テーマ着色用）
      _iconMailAddress = (Bitmap)pictureBoxMailAddress.Image;
      _iconCheckIcon = (Bitmap)pictureBoxCheckIcon.Image;
      _iconLockIcon = (Bitmap)pictureBoxLockIcon.Image;
      _iconInvalidIcon = (Bitmap)pictureBoxInvalid.Image;
      _iconKeyIcon = (Bitmap)pictureBoxKey.Image;
    }
    ...
    /// <summary>
    /// 全アイコンを現在のテーマ＋選択状態に従って再描画する。
    /// テーマ切り替え時や初期化時に呼び出す。
    /// TODO: アイコンの種類が増えてきたら、ThemeIconManager にアイコンIDを追加して Get メソッドで一括管理する方式にリファクタリングする。
    /// </summary>
    private void RefreshAllIcons()
    {
      if (_iconEncrypt == null) return; // デザイナーまたは未初期化時はスキップ

      // サイドバーメニューアイコン（panelSidebarStart）
      pictureBoxAtc.Image = ThemeIconManager.Get(_iconEncrypt, _atcChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      pictureBoxRsa.Image = ThemeIconManager.Get(_iconPublicKey, _rsaChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      pictureBoxPostPpap.Image = ThemeIconManager.Get(_iconPostPpap, _exeChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      pictureBoxDec.Image = ThemeIconManager.Get(_iconDecrypt, _decChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      ...
      pictureBoxDecryptBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
      pictureBoxProgressStateBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
      pictureBoxRsaBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
      pictureBoxRsaKeyBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
      pictureBoxPostPpapRegisterBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);
      pictureBoxPostPpapBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Normal);

      // 設定ボタン
      pictureBoxOption.Image = ThemeIconManager.Get(_iconSettings, ThemeIconManager.IconState.Normal);

      // サイドパネルのアイコン（panelSidebar*, panelHeader*）
      pictureBoxPostPpapPage.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Normal);
      pictureBoxPostPpapMain.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Normal);
      UpdateSidePanelIcons();

      // Post-PPAP パネルのメールアドレスアイコン
      if (_iconMailAddress != null)
        pictureBoxMailAddress.Image = ThemeIconManager.Get(_iconMailAddress, ThemeIconManager.IconState.Normal);

      // Post-PPAP 検索結果アイコン（表示中の場合のみ再着色）
      if (pictureBoxValid.Visible && _postPpapSearched)
      {
        pictureBoxValid.Image = _foundRecipientKey != null
            ? ThemeIconManager.Get(_iconCheckIcon, ThemeIconManager.IconState.Normal)
            : ThemeIconManager.Get(_iconInvalidIcon, ThemeIconManager.IconState.Normal);
        pictureBoxValidDetails.Image = _foundRecipientKey != null
            ? ThemeIconManager.Get(_iconLockIcon, ThemeIconManager.IconState.Normal)
            : ThemeIconManager.Get(_iconKeyIcon, ThemeIconManager.IconState.Normal);
      }

      // パスワード入力欄の eye/blind アイコン
      textBoxPassword.RefreshTheme();
      textBoxRePassword.RefreshTheme();
      textBoxDecryptPassword.RefreshTheme();
    }

    /// <summary>
    /// サイドバーアイコンの選択状態をすべてリセットする。
    /// </summary>
    ...
    /// <summary>
    /// サイドバーアイコンの選択状態に応じてすべてのアイコン画像を更新する。
    /// </summary>
    private void UpdateSidebarIcons()
    {
      pictureBoxAtc.Image = ThemeIconManager.Get(_iconEncrypt,
          _atcChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      pictureBoxRsa.Image = ThemeIconManager.Get(_iconPublicKey,
          _rsaChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      pictureBoxPostPpap.Image = ThemeIconManager.Get(_iconPostPpap,
          _exeChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
      pictureBoxDec.Image = ThemeIconManager.Get(_iconDecrypt,
          _decChecked ? ThemeIconManager.IconState.Active : ThemeIconManager.IconState.Normal);
    }

    /// <summary>
    /// 暗号化/復号パネルのサイドアイコンを現在の暗号化タイプに合わせて更新する。
    /// </summary>
    private void UpdateSidePanelIcons()
    {
      if (AppSettings.Instance.EncryptionFileType == FILE_TYPE_ATC_EXE)
      {
        pictureBoxEncryption.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Normal);
      ...
      pictureBoxEncryptionConfirm.Image = pictureBoxEncryption.Image;
      pictureBoxDecryption.Image = ThemeIconManager.Get(_iconDecrypt, ThemeIconManager.IconState.Normal);
      pictureBoxProgress.Image = pictureBoxEncryption.Image;
    }

    //----------------------------------------------------------------------
    // 半透明バックドロップ効果の適用
    // ダークモード: Acrylic（ブラー + 半透明）— DWM SetWindowCompositionAttribute
    // ライトモード: Form.Opacity（均一半透明）
    // ウィンドウが表示された後に呼び出す必要がある
    //----------------------------------------------------------------------
    private void ApplyBackdrop(bool fDark)
    {
      if (!this.IsHandleCreated) return;

      if (fDark)
      {
        // --- ダークモード: Acrylic ブラー + 半透明 ---
        this.Opacity = 1.0;

        if (Environment.OSVersion.Version.Major >= 10)
        {
          var user32Path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "user32.dll");
          using (var user32Dll = new UnManagedDll(user32Path))
          {
            try
            {
              var setWindowComposition = user32Dll.GetProcDelegate<SetWindowCompositionAttributeDelegate>("SetWindowCompositionAttribute");

              var accent = new AccentPolicy();
              accent.AccentState = 4; // ACCENT_ENABLE_ACRYLICBLURBEHIND
              accent.AccentFlags = 2; // ACCENT_FLAG_DRAW_ALL
              // #070f1b, 50% 不透明 → AABBGGRR = 0x801B0F07
              accent.GradientColor = unchecked((int)0x801B0F07);
              ...
              try
              {
                ...
                setWindowComposition(this.Handle, ref data);
              }
              finally
              {
      ...
      {
        // --- ライトモード: Acrylic 無効化 + Form.Opacity ---
        // ダーク→ライト切替時に Acrylic を無効化
        if (_backdropEnabled && Environment.OSVersion.Version.Major >= 10)
        {
          var user32Path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "user32.dll");
          using (var user32Dll = new UnManagedDll(user32Path))
          {
        ...
        // 背景色 #EEF3F8 + Opacity 0.93 で「透過感の知覚限界」を狙う
    ...
    private static string CreateKeyPair(string filePath, string guidString)
    {
      ...
      //-----------------------------------
      //RSACryptoServiceProviderオブジェクトの作成（RSA-4096）
    ...
    {
      pictureBoxRsaBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Hover);
    }
    ...
    {
      pictureBoxRsaKeyBackButton.Image = ThemeIconManager.Get(_iconBack, ThemeIconManager.IconState.Hover);
    }
    ...
    /// <summary>
    /// Acrylic バックドロップ有効時、フォーム背景の描画をスキップする。
    /// GDI で上書きしないことで、DWM の Acrylic ブラー効果を透過させる。
    /// </summary>
    protected override void OnPaintBackground(PaintEventArgs e)
    {
      if (_backdropEnabled)
      {
        // 描画しない → DWM の Acrylic がそのまま表示される
        return;
      }
    ...
    private CancellationTokenSource _postPpapCts;
    private bool _postPpapRegistrationComplete = false;

    // Post-PPAP 公開鍵検索の状態
    private PostPpapKeyInfo _foundRecipientKey = null;  // 検索で見つかった相手の公開鍵
    private bool _postPpapSearched = false;              // 検索済みフラグ
    private bool _isSearching = false;                   // 検索中フラグ（二度押し防止）

    // Post-PPAP 秘密鍵（復号用、公開鍵暗号ファイルの自動復号に使用）
    private Org.BouncyCastle.Crypto.Parameters.RsaPrivateCrtKeyParameters _postPpapPrivateKey = null;
    private Org.BouncyCastle.Crypto.Parameters.MLKemPrivateKeyParameters _postPpapMlKemPrivateKey = null;

    /// <summary>
    /// サイドバーの Post-PPAP アイコンクリック。
    /// 登録済み → PostPpapMain、未登録 → PostPpapRegister を表示。
    /// </summary>
    private void pictureBoxPostPpap_Click(object sender, EventArgs e)
    {
      ClearAllChecked();
      _exeChecked = true;
      UpdateSidebarIcons();
      ...
      if (PostPpapManager.HasAccounts)
      {
        ResetPostPpapMainState();
        panelPostPpapMain.Visible = true;
      }
      else
        panelPostPpapRegister.Visible = true;
    }

    private void pictureBoxPostPpap_MouseEnter(object sender, EventArgs e)
    {
      if (!_exeChecked)
        pictureBoxPostPpap.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Hover);
    }

    private void pictureBoxPostPpap_MouseLeave(object sender, EventArgs e)
    {
      if (!_exeChecked)
        pictureBoxPostPpap.Image = ThemeIconManager.Get(_iconPostPpap, ThemeIconManager.IconState.Normal);
    }

    /// <summary>
    /// Post-PPAP ページの戻るボタン — スタートページに戻る。
    /// Register / Main 両方から共用。
    /// </summary>
    private void pictureBoxBackToMain_Click(object sender, EventArgs e)
    {
      _postPpapCts?.Cancel();
    ...
    private void textBoxYourMailAddress_KeyDown(object sender, KeyEventArgs e)
    {
      if (e.KeyCode == Keys.Enter)
      {
        e.Handled = true;
        e.SuppressKeyPress = true;
        buttonRegisterYourMailAddress.PerformClick();
      }
    }

    private void textBoxPuclicMailAddress_KeyDown(object sender, KeyEventArgs e)
    {
      if (e.KeyCode == Keys.Enter)
      {
        e.Handled = true;
        e.SuppressKeyPress = true;
        buttonPuclicMailAddressSearch.PerformClick();
      }
    }

    /// <summary>
    /// 「登録する」/「次へ」ボタン。
    /// 登録完了後は「次へ」に変わり、PostPpapMain へ遷移する。
    /// </summary>
    private async void buttonRegisterYourMailAddress_Click(object sender, EventArgs e)
    {
      // 登録完了後の「次へ」→ メインページへ遷移
      if (_postPpapRegistrationComplete)
      {
        panelPostPpapRegister.Visible = false;
        panelPostPpapMain.Visible = true;
        return;
      ...
      labelPostPpapStatus.Visible = true;

      var email = textBoxYourMailAddress.Text.Trim();
      if (string.IsNullOrEmpty(email))
      {
        labelPostPpapStatus.ForeColor = Color.FromArgb(200, 60, 60);

        // メールアドレスを入力してください
        // ---
        // Enter email address
        labelPostPpapStatus.Text = Resources.ProgressMessageEnterEmailAddress;
        return;
      }

      try
      {
        buttonRegisterYourMailAddress.Enabled = false;
        textBoxYourMailAddress.Enabled = false;
        progressBarPostPpap.Visible = true;
        labelPostPpapStatus.ForeColor = Color.Gray;

        // 確認メールを送信中...
        // ---
        // Sending confirmation email...
        labelPostPpapStatus.Text = Resources.ProgressMessageSendingConfirmationEmail;

        var reg = await PostPpapManager.StartRegistrationAsync(email);
        ...
        labelPostPpapStatus.Text = Resources.ProgressMessageConfirmationEmailSent;

        _postPpapCts = new CancellationTokenSource();
        var verified = await PostPpapManager.AwaitVerificationAsync(
            reg.PollToken, reg.Account, timeoutMinutes: 10, cancellationToken: _postPpapCts.Token);

        if (verified)
        {
          labelPostPpapStatus.ForeColor = Color.FromArgb(0, 128, 64);

          // 登録が完了しました
          // ---
          // Registration complete
          labelPostPpapStatus.Text = Resources.ProgressMessageRegistrationComplete;
          progressBarPostPpap.Visible = false;
          await Task.Delay(1500);

          // ボタンを「次へ」に変更
          _postPpapRegistrationComplete = true;
          buttonRegisterYourMailAddress.Text = "次へ";
          buttonRegisterYourMailAddress.Enabled = true;
        }
        else
        {
          labelPostPpapStatus.ForeColor = Color.FromArgb(200, 60, 60);
          ...
          // Timed out
          // Please try again
          labelPostPpapStatus.Text = Resources.ProgressMessageTimeout;
          buttonRegisterYourMailAddress.Enabled = true;
          textBoxYourMailAddress.Enabled = true;
        }
      }
      catch (OperationCanceledException)
      {
        ...
        labelPostPpapStatus.Text = Resources.ProgressMessageRegistrationCanceled;
        buttonRegisterYourMailAddress.Enabled = true;
        textBoxYourMailAddress.Enabled = true;
      }
      catch (Exception ex)
      {
        labelPostPpapStatus.ForeColor = Color.FromArgb(200, 60, 60);
        labelPostPpapStatus.Text = "エラー: " + ex.Message;
        buttonRegisterYourMailAddress.Enabled = true;
        textBoxYourMailAddress.Enabled = true;
      }
      finally
    ...
    /// <summary>
    /// 公開鍵検索ボタン。メールアドレスでキーサーバーを検索する。
    /// </summary>
    private async void buttonPuclicMailAddressSearch_Click(object sender, EventArgs e)
    {
      if (_isSearching) return;

      var email = textBoxPuclicRecipientMailAddress.Text.Trim();
      if (string.IsNullOrEmpty(email))
      {
        // Enter email address
        // ---
        // メールアドレスを入力してください
        labelValidMessage.Text = Resources.ProgressMessageEnterEmailAddress;
        labelValidMessage.ForeColor = Color.FromArgb(200, 60, 60);
        labelValidMessage.Visible = true;
        return;
      }

      // 検索中の UI 状態（二度押し防止）
      _isSearching = true;
      buttonPuclicMailAddressSearch.ForeColor = _isDarkTheme ? Color.FromArgb(90, 90, 90) : Color.Gray;
      textBoxPuclicRecipientMailAddress.Enabled = false;
      labelValidMessage.Text = "検索中...";
      labelValidMessage.ForeColor = Color.Gray;
      labelValidMessage.Visible = true;
      labelValidDetails.Visible = false;
      panelPublicDragAndDrop.Visible = false;
      pictureBoxValid.Visible = false;
      ...
      {
        _postPpapCts = new CancellationTokenSource();
        _foundRecipientKey = await PostPpapManager.LookupPublicKeyAsync(email, _postPpapCts.Token);
        _postPpapSearched = true;
        ...
        if (_foundRecipientKey != null)
        {
          ...
          // ---
          // パスワード不要で安全に送信可能
          labelValidMessage.Text = Resources.DialogMessageRecipientRegistered;
          labelValidMessage.ForeColor = Color.FromArgb(0, 128, 64);
          labelValidMessage.Visible = true;

          // パスワード不要で安全に送信可能
          // ---
          // Send securely without a password
          labelValidDetails.Text = Resources.DialogMessageSendWithoutPassword;
          labelValidDetails.ForeColor = Color.FromArgb(0, 128, 64);
          labelValidDetails.Visible = true;

          // pictureBoxValid に pictureBoxCheckIcon の画像を設定して表示
          pictureBoxValid.Image = ThemeIconManager.Get(_iconCheckIcon, ThemeIconManager.IconState.Normal);
          pictureBoxValid.Visible = true;
          pictureBoxInvalid.Visible = false;

          // pictureBoxValidDetails に pictureBoxLockIcon の画像を設定して表示
          pictureBoxValidDetails.Image = ThemeIconManager.Get(_iconLockIcon, ThemeIconManager.IconState.Normal);
          pictureBoxValidDetails.Visible = true;

          // ドラッグ＆ドロップエリアを表示
          panelPublicDragAndDrop.Top = labelValidDetails.Top + labelValidDetails.Height + 16;
          panelPublicDragAndDrop.Visible = true;
          panelPostPpapMain.Invalidate();

          // 暗号化するファイルまたはフォルダーを
          // ここにドラッグ＆ドロップしてください。
          // ---
          // Drag and drop files or folders to encrypt here.
          labelPasswordSharingIfRegistered.Text = Resources.LabelPasswordSharingRequired;
          labelPasswordSharingIfRegistered.Visible = true;
        ...
        else
        {
          ...
          // ---
          // Not Registered
          labelValidMessage.Text = Resources.DialogMessageRecipientNotRegistered;
          labelValidMessage.ForeColor = Color.FromArgb(200, 120, 0);
          labelValidMessage.Visible = true;

          // パスワード方式で暗号化
          // ---
          // Encrypted with standard password-based encryption
          labelValidDetails.Text = Resources.DialogMessagePasswordEncryptionStandard;
          labelValidDetails.ForeColor = Color.FromArgb(200, 120, 0);
          labelValidDetails.Visible = true;

          // pictureBoxValid に pictureBoxInvalid の画像を設定して表示
          pictureBoxValid.Image = ThemeIconManager.Get(_iconInvalidIcon, ThemeIconManager.IconState.Normal);
          pictureBoxValid.Visible = true;
          pictureBoxInvalid.Visible = false;

          // pictureBoxValidDetails に pictureBoxKey の画像を設定して表示
          pictureBoxValidDetails.Image = ThemeIconManager.Get(_iconKeyIcon, ThemeIconManager.IconState.Normal);
          pictureBoxValidDetails.Visible = true;

          // 通常の暗号化方式で送信します。
          // 送信後、パスワードを共有する必要があります。
          // 
          // メールアドレス登録済みの相手には、パスワードなしで送信できます。
          // 
          // ここに暗号化するファイルまたはフォルダーをドラッグ＆ドロップしてください。
          // ---
          ...
          labelPasswordSharingIfRegistered.Visible = true;
          // ドラッグ＆ドロップエリアを表示
          panelPublicDragAndDrop.Top = labelPasswordSharingIfRegistered.Top + labelPasswordSharingIfRegistered.Height + 16;
          panelPublicDragAndDrop.Visible = true;
        }
      ...
      catch (OperationCanceledException)
      {
        ...
        labelValidMessage.Text = Resources.LabelSearchCanceled;
        labelValidMessage.ForeColor = Color.Gray;
        labelValidMessage.Visible = true;
        _postPpapSearched = false;
        // 暗号化ファイル送信後、パスワードの共有が必要です
        // メールアドレス登録済みの相手には、パスワード不要で送信できます
        labelPasswordSharingIfRegistered.Visible = false;
      ...
      catch (Exception ex)
      {
        ...
        // ---
        // Error:
        labelValidMessage.Text = Resources.LabelError +  ex.Message;
        labelValidMessage.ForeColor = Color.FromArgb(200, 60, 60);
        labelValidMessage.Visible = true;
        _postPpapSearched = false;
        // 暗号化ファイル送信後、パスワードの共有が必要です
      ...
      {
        _isSearching = false;
        buttonPuclicMailAddressSearch.ForeColor = _isDarkTheme
            ? Color.White : SystemColors.ControlText;
        textBoxPuclicRecipientMailAddress.Enabled = true;
      }
    }

    /// <summary>
    /// panelPublicDragAndDrop のマウスエンターイベント（カラーチェンジ）。
    /// </summary>
    private void panelPublicDragAndDrop_MouseEnter(object sender, EventArgs e)
    {
      if (!_isDarkTheme)
        panelPublicDragAndDrop.BackColor = LightHoverColor;
      else
        panelPublicDragAndDrop.BackColor = DarkHoverColor;
    }

    /// <summary>
    /// panelPublicDragAndDrop のマウスリーブイベント（カラーリセット）。
    /// </summary>
    private void panelPublicDragAndDrop_MouseLeave(object sender, EventArgs e)
    {
      panelPublicDragAndDrop.BackColor = Color.Transparent;
    }

    /// <summary>
    /// Post-PPAP: 公開鍵による暗号化処理を開始する。
    /// _foundRecipientKey が設定済みの状態で呼ぶこと。
    /// EncryptionProcess() 内で _foundRecipientKey から公開鍵が AddRecipient される。
    /// </summary>
    private void StartPostPpapEncryption()
    {
      if (_foundRecipientKey == null || AppSettings.Instance.FileList.Count == 0)
        return;
      ...
      // パスワード入力をスキップして直接暗号化処理へ
      FileIndex = 0;
      EncryptionProcess();
    }

    /// <summary>
    /// Post-PPAP メイン画面を表示する際に検索状態をリセットする。
    /// </summary>
    private void ResetPostPpapMainState()
    {
      _foundRecipientKey = null;
      _postPpapSearched = false;
      labelValidMessage.Visible = false;
      labelValidDetails.Visible = false;
      labelPasswordSharingIfRegistered.Visible = false;
      ...
      pictureBoxValid.Visible = false;
      pictureBoxValidDetails.Visible = false;
      textBoxPuclicRecipientMailAddress.Text = "";

      // 登録済みアカウントのメールアドレスを表示
      var myAccount = PostPpapManager.LoadKeyInfo();
      labelRegisteredMailAddress.Text = myAccount?.Email ?? "";
      labelRegisteredMailAddress.Visible = !string.IsNullOrEmpty(myAccount?.Email);
      pictureBoxMailAddress.Visible = !string.IsNullOrEmpty(labelRegisteredMailAddress.Text);
    }

    #endregion

    private void panelRsaKey_VisibleChanged(object sender, EventArgs e)
    {
      if (!panelRsaKey.Visible) return;
      BeginInvoke(new Action(() => LayoutActivePanel()));
    }

    private void panelPostPpapRegister_VisibleChanged(object sender, EventArgs e)
    {
      if (!panelPostPpapRegister.Visible) return;
      BeginInvoke(new Action(() => LayoutActivePanel()));
    }

    private void panelPostPpapMain_VisibleChanged(object sender, EventArgs e)
    {
      if (!panelPostPpapMain.Visible) return;
      BeginInvoke(new Action(() => LayoutActivePanel()));
    }
    ...
    // RSA コマンドライン実行
    //======================================================================
    #region RSA command-line

    /// <summary>
    /// /genrsa コマンド: RSA-4096 鍵ペアを生成して終了。
    /// </summary>
    private void ExecuteRsaGenerateKeyFromCommandLine()
    {
      var outDir = AppSettings.Instance.GenerateRsaKeyOutputDir;
      ...
      var filePath = Path.Combine(outDir, "AttacheCase_RSA");
      CreateKeyPair(filePath, "");
      Console.WriteLine("RSA-4096 key pair generated:");
      Console.WriteLine("  Public key:  " + filePath + ".atcpub");
      Console.WriteLine("  Private key: " + filePath + ".atcpvt");

      if (AppSettings.Instance.fEndToExit)
        Application.Exit();
    }
    ...
    // Post-PPAP コマンドライン実行
    //======================================================================
    #region Post-PPAP command-line

    /// <summary>
    /// /p-ppap コマンド: キーサーバから公開鍵を検索し、Post-PPAP 暗号化を実行する。
    /// 使用例: AttacheCase.exe /p-ppap=m@hibara.org /exit=1 file.txt
    /// </summary>
    private async void ExecutePostPpapEncryptFromCommandLine()
    {
      var email = AppSettings.Instance.PostPpapRecipientEmail;

      // プログレス画面を表示してキーサーバ検索中であることを示す
      panelStartPage.Visible = false;
      panelProgressState.Visible = true;
      labelCryptionType.Text = "Post-PPAP";
      labelProgressMessageText.Text = "Searching: " + email;
      progressBar.Style = ProgressBarStyle.Marquee;

      try
      {
        _foundRecipientKey = await PostPpapManager.LookupPublicKeyAsync(email);
      }
      catch (Exception ex)
      {
        MessageBox.Show("Post-PPAP key lookup failed:\n" + ex.Message,
          Resources.DialogTitleAlert, MessageBoxButtons.OK, MessageBoxIcon.Error);
        if (AppSettings.Instance.fEndToExit) Application.Exit();
        return;
      }

      if (_foundRecipientKey == null)
      {
        MessageBox.Show("Post-PPAP: Public key not found for: " + email,
          Resources.DialogTitleAlert, MessageBoxButtons.OK, MessageBoxIcon.Warning);
        if (AppSettings.Instance.fEndToExit) Application.Exit();
        return;
      }

      // 検索成功 → 暗号化処理へ（既存フローに合流）
      progressBar.Style = ProgressBarStyle.Continuous;
      FileIndex = 0;
      EncryptionProcess();
    }
